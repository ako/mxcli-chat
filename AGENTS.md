...
```bash
./mxcli -p MxcliChat.mpr    # Correct - uses local binary
```
...
```bash
./mxcli setup mxbuild -p MxcliChat.mpr    # Auto-download for project's Mendix version
```

After setup, `mx` is at `~/.mxcli/mxbuild/{version}/modeler/mx`. Usage:

```bash
~/.mxcli/mxbuild/*/modeler/mx check MxcliChat.mpr   # Validate project
./mxcli docker check -p MxcliChat.mpr               # Alternative (auto-downloads mxbuild)
```
...
```bash
./mxcli -p MxcliChat.mpr -c "SHOW MODULES"              # List all modules
./mxcli -p MxcliChat.mpr -c "SHOW STRUCTURE"             # Project overview
./mxcli -p MxcliChat.mpr -c "SHOW ENTITIES IN MyModule"  # Entities in a module
./mxcli -p MxcliChat.mpr -c "DESCRIBE ENTITY MyModule.Customer"  # Entity details
```

### Execute an MDL Script File

```bash
./mxcli exec script.mdl -p MxcliChat.mpr
```
...
```bash
./mxcli
# Then: CONNECT LOCAL 'MxcliChat.mpr';
```
...
./mxcli check script.mdl -p MxcliChat.mpr --references  # With reference validation
```
...
```bash
./mxcli check script.mdl -p MxcliChat.mpr --references
```
...
```bash
# Lint the project
./mxcli lint -p MxcliChat.mpr

# With colored output
./mxcli lint -p MxcliChat.mpr --color

# List available rules
./mxcli lint -p MxcliChat.mpr --list-rules

# Output as SARIF
./mxcli lint -p MxcliChat.mpr --format sarif > results.sarif
```
...
./mxcli report -p MxcliChat.mpr

# JSON report
./mxcli report -p MxcliChat.mpr --format json

# HTML report
./mxcli report -p MxcliChat.mpr --format html
```
